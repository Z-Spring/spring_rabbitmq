...
public class UserController {
    ...
    public ModelAndView base(Model model, HttpServletRequest request, HttpSession session)
            throws IOException {
        if (session.getAttribute("user") != null) {
            ...
            model.addAttribute(
                    "price",
                    NumberFormat.getCurrencyInstance(Locale.CHINA).format(bookService.getPrice(uid)));
        ...
        if (judgeRegex(email)) {
            ...
            if (getUserFromRedis(name) == null) {
                ...
                if (userService.login(user)) {
                    ...
                    //
                    // queueMessageListener.onLoginMessageFromMailQueue(LoginMessage.of(user.getEmail(),
                    // user.getName(), true));
    ...
    /**
     * key :String value: Json 登录时，先从缓存中获取信息，如果缓存中没有的话，再从数据库中获取
     */
    ...
    public ModelAndView doRegister(
        ...
        if (putNameToRedis(name)) {
            ...
            // todo:hashmap括号中填写什么？2022.4.27
            Map<String, String> model = new HashMap();
            model.put("username_repeat", "用户名重复！请重新输入");
            return new ModelAndView("register", model);
        }
    }

    /**
     * 将注册的用户名放到Redis中，来识别注册的用户名是否重复 之所以没有在注册方法那里设置，是因为user数据库中uid是主键
     *
     * @param name 用户名
     */
